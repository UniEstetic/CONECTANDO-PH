...
import {selectProvider, validateLogin, getProfile} from "@/app/lib/services/auth.service";
        ...
        try{
          ...
          if(providerTk?.result?.authorization?.token){
            ...
            if (startLogin?.result?.access_token) {
              ...
              const profileData = await getProfile(String(access_token_backend));

              console.log("credentials", credentials, profileData);
              return {
                ...profileData,
                id: profileData.id || "ID_UNICO",
